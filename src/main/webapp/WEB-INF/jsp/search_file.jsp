...
<div class="modal-dialog" role="document">
                                ...
                                <c:forEach var="record" items="${pendingWithBranchApplicationForm}">
                                    ...
                                    <div class="elements data-row row-23818">
                                        <div class="row tr">
                                            ...
                                            <c:if test="${login_users.userSession ne record.createdBy && record.status ne 'rejected' && login_users.depname ne  'OS' && login_users.depname ne 'fedserv'}">

                                            	<c:if  test="${record.status eq 'new'}">
	                                                <div class="col-1 col-md-1 col">   
                                                ...
                                                <c:if  test="${record.status eq 'pending' || record.status eq 'modified'}">
                                                <div class="col-1 col-md-1 col">
                                                	<form id="approveapplnviewform${record.id}" method="post" action="pendingwithfinalapplication">
                                                        <input type="hidden"  id="applnformid" name="applnformid" value="${record.id}">
                                                        <button class="btn btnspec btn-primary" >Approve</button>
                                                    </form>                                       
                                                </div>
                                                </c:if>
                                            	<c:if  test="${record.status eq 'approved'}">   
                                                <div class="col-1 col-md-1 col">
                                                	<form id="modifiedapplnviewform${record.id}" method="post" action="modified-application">
                                                        <input type="hidden"  id="applnformid" name="applnformid" value="${record.id}">
                                                        <button class="btn btnspec btn-primary"  >Modify</button>
                                                    </form>                                       
                                                </div>
                                                </c:if>
                                        ...
                                        <div class="row">
                                            <div class="col-auto mr-auto">
                                                ...
                                                <c:if  test="${record.status eq 'modified' && not empty  record.modifiedBy}">
	                                                <span class="badge badge-primary">Modified By :
	                                                    ${record.modifiedBy}</span>
	                                            </c:if>
	                                            <c:if  test="${record.status eq 'new' && not empty  record.modifiedBy}">
	                                                < <span class="badge badge-danger">Application resubmitted by OPD. For details, click on View</span>>
	                                            </c:if>
                                                <c:if test="${not empty  record.approvedBy && record.status eq 'approved'}">
                                                    <span class="badge badge-success">Approved By :
                                                        ${record.approvedBy}</span>
                                                </c:if>
                                                <c:if test="${not empty  record.rejectedBy && record.status eq 'rejected'}">
                                                <span class="badge badge-danger">Approved By :
                                                    ${record.rejectedBy}</span>
                                            </c:if>
  ...
  <script>
  var text1="${search_status}";
jQuery(document).ready(function($){
$('select').find('option[value="'+text1+'"]').attr('selected','selected');
});
  </script>
